 ...
 * `enableFileTracking()` - By default, Matomo disable `file://` protocal tracking to prevent local page send tracking stats. To enable it, simply add `enableFileTracking`   [Learn more about enable file tracking](https://matomo.org/faq/how-to-enable-file-protocal-tracking)
