...
* `disableCampaignParameters()` - By default, Matomo will send campaign parameters (mtm, utm etc) to the tracker and record that information. Some privacy regulations may not allow for this information to be collected unless there is consent given. If this is used in a combination with `requireConsent` then once tracking consent has been given the tracker will start collecting this information. If it's used alone then campaign parameters will never be sent.
* `enableCampaignParameters()` - Enable campaign parameters being sent to Matomo if you previously disabled it.
