...
* `bw_bytes` &mdash; Attach the byte count for an action to a Tracking API request when attributing network usage and costs or validating optimisation impact. Send actual bytes (for downloads, the file size; for pageviews, a measured transfer size). Read more about [monitoring bandwidth](https://plugins.matomo.org/Bandwidth#faq).
