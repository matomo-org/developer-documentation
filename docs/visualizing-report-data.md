...
To change exactly what data is loaded, visualizations should alter the request by setting the [request_parameters_to_modify](#) request config property within the [Visualization::beforeLoadDataTable](#) method. For example:
...
_Note: If you change what data is loaded, you may also need to override the [Visualization::isThereDataToDisplay](#) method. Otherwise the [no data message](#) may not appear if there is no data in the report you are displaying._
TODO: no data message should link to image?
...
_Note: Report data can also be manipulated by setting the [filters](#) display property._
...
After you extend the class, you must notify [Visualization](#) of the new class by setting the [datatable_js_type](#) display property in your visualization. For example:
...
You can also use the [datatable_css_class](#) property to add more CSS classes if you need to. This property can be useful if you need to customize the way visualizations appear for specific reports.
