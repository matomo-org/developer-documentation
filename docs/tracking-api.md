...
To track page views, events, visits, you have to send a HTTP request to your Tracking REST API endpoint: <span style="color: #008000;">**http://your-piwik-domain.tld/piwik.php**</span>
...
*   <span style="color: #000080;">Required and recommended parameters</span>
    ...
    *   **rand**** (recommended)** &mdash; A random parameter, for example a random number generated before each request — this is useful to avoid the tracking request being cached by the browser or a proxy.
    *   **apiv** ** (recommended)** &mdash; The parameter &amp;apiv=1 defines the api version you want to use.

*   <span style="color: #000080;">Optional visitor info</span> — we recommend to set these values if they are available and relevant to your use case.
    ...
    *   **res** &mdash; The resolution of the device, eg _1280×1024_
    ...
    *   **lang — **This value is the Accept-Language HTTP header, which lets Piwik detect user country (based on language) if [GeoIP](http://piwik.org/faq/troubleshooting/#faq_65) is not enabled.

        Note: Alternatively you can set the Accept-Language HTTP header instead of using the &amp;lang= parameter.

*   <span style="color: #000080;">Optional action/event info</span> — we recommend to set these values if they are available and relevant to your use case.
...
*   <span style="color: #000080;">Special parameters requiring authentication via token_auth</span>
    ...
    *   **cip** &mdash; (requires token_auth to be set) defines the visitor IP — ipv4 and ipv6 notations supported.
...
<span style="color: #333333; font-family: courier;">http://piwik-server/piwik.php?_cvar={“1″:["OS","iphone 5.0"],”2″:["Piwik Mobile Version","1.6.2"],”3″:["Locale","en::en"],”4″:["Num Accounts","2"]}&amp;action_name=View settings&amp;url=http://mobileapp.piwik.org/window/settings &amp;idsite=8876&amp;rand=351459&amp;h=18&amp;m=13&amp;s=3 &amp;rec=1&amp;apiv=1&amp;cookie= &amp;urlref=http://iphone.mobileapp.piwik.org&amp;_id=af344a398df83874 &amp;_idvc=19&amp;res=320×480&amp;</span>
...
<span style="color: #008000;">**Tracking requests will then output the tracking logs rather than displaying a 1*1 transparent GIF beacon.**</span>
