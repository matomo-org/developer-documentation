...
Before releasing a new version we release an RC. As soon as an RC has been released we only merge regressions. Very important bug fixes can be merged after an RC too if their risk for side effects is low. No other changes should be merged between an RC anf the release. We usually run the RC for a minor release for at least a week, for major releases multiple weeks and for patch releases at least one day but better multiple days.
