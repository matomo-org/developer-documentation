...
*   **keep_flattened_dimension_columns**; when a report is requested with `flat=1&show_dimensions=1`, one column is added per dimension. Those column names only exist after flattening, so they cannot be listed in a `showColumns` allowlist and would be dropped. Set `keep_flattened_dimension_columns=1` to re-add them after flattening. Defaults to `0`, so `showColumns` on its own behaves as before. (Available as of Matomo 5.13.0)

*   **percent_of_total**; report rows include a `{metric}_percent_of_total` column (eg. `nb_visits_percent_of_total`) for each metric the report processes totals for. The values follow `format_metrics` like other percent metrics and are included in all export formats. Set `percent_of_total=0` to leave them out. Unique visitors and users are excluded, as a report total is not a meaningful denominator for them, and the columns are not added when `totals=0`, when the report has no dimension (eg. `VisitsSummary.get`), or when `pivotBy` is used. **Note for CSV/TSV consumers parsing columns by position:** these columns change the header and column count, so pass `percent_of_total=0` to keep the previous output. (Available as of Matomo 6.0.0)
