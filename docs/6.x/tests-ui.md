...
If you want to use a different headless browser executable that is run by Puppeteer during the UI tests,
you may want to add a custom browser config object to `tests/UI/config.js`, such as
```
...
*Note: If the `tests/UI/config.js` file does not exist, it is automatically generated when running `tests:run-ui`
console command for the fist time, and a code comment `File generated by the tests:run-ui command` is added.
Ensure the code comment is removed for your customisations to persists.*
 ...
 * [Overlay_spec.js](https://github.com/matomo-org/matomo/blob/6.x-dev/plugins/Overlay/tests/UI/Overlay_spec.js)
 * [Dashboard_spec.js](https://github.com/matomo-org/matomo/blob/6.x-dev/plugins/Dashboard/tests/UI/Dashboard_spec.js)
 * [Login_spec.js](https://github.com/matomo-org/matomo/blob/6.x-dev/plugins/Login/tests/UI/Login_spec.js)
...
The `tests:sync-ui-screenshots` console command can be used to speed up the process. Run `./console tests:sync-ui-screenshots -h` to learn more.
