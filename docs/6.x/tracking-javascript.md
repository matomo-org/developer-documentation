...
* `enableHeartBeatTimer( activeTimeInseconds )` - Enable a heartbeat timer that sends additional requests to Matomo to improve the accuracy of time spent in a visit. These requests are sent only while the page is active and in focus. They do not track additional actions or pageviews. By default, `activeTimeInSeconds` is set to 15 seconds with a minimum of 5 seconds. The heartbeat timer does not send ping requests at fixed intervals. Instead, Matomo sends a ping request only after the visitor has actively viewed the page for at least the configured time and then leaves the page or switches focus away from the tab. See also `ping` and the [developer guide](https://developer.matomo.org/guides/tracking-javascript-guide#accurately-measure-the-time-spent-on-each-page). 
...
*   `setIgnoreCampaignAttributionForSources( string | array )` - Set sources whose campaign values in the current URL should be ignored for attribution. Matching campaign parameters are still kept in the tracked URL/request. This method is available as of Matomo 5.12.
*   `getIgnoreCampaignAttributionForSources()` - Retrieve the sources whose campaign values should be ignored for attribution. This method is available as of Matomo 5.12.
