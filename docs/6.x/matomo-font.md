...
We use a custom Matomo font in our UI to show icons. You can [find the font in the repository as part of the Morpheus](https://github.com/matomo-org/matomo/tree/6.x-dev/plugins/Morpheus/fonts) theme.
...
* Download this file (use "File -> Save" in the browser): [selection.json](https://raw.githubusercontent.com/matomo-org/matomo/6.x-dev/plugins/Morpheus/fonts/selection.json).
...
* Update [plugins/Morpheus/stylesheets/base/icons.css](https://github.com/matomo-org/matomo/blob/6.x-dev/plugins/Morpheus/stylesheets/base/icons.css) 
...
* Update [plugins/Morpheus/Controller.php](https://github.com/matomo-org/matomo/blob/6.x-dev/plugins/Morpheus/Controller.php#L746) by adding or removing the name of the changed icon in one of the available categories. If the icon is not appearing on the page, try clearing the cache `./console cache:clear`.
...
* Update our [LEGALNOTICE](https://github.com/matomo-org/matomo/blob/6.x-dev/LEGALNOTICE) and mention or remove the name of the icon we added, changed or removed. You already find a list of icons we've been using so far in this file.
