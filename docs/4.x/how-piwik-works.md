...
During tracking not all plugins are loaded, only the plugins that are identified as being needed during tracking (partially Matomo detects this automatically based on what events they listen to or whether they define dimensions) and some plugins mark themselves as a tracker plugin specifically.

Any tracked data is stored in `log_*` tables. These tables store all the raw data which is then later aggregated to report archives see below. Basically for each visit and for each action a visitor takes a new row is created in their respective log tables. Some log tables such as `log_visit` are also updated during a tracking request.  
...
Matomo Core only defines the main processes and behaviors. Plugins can extend and customize them through several *extensibility points*:
