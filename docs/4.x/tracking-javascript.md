...
*   `setVisitStandardLength( visitLengthInSeconds )` - Sets the visit standard length (in seconds). This should ideally match the `visit_standard_length` setting in Matomo in case you have [customised](https://matomo.org/faq/general/faq_36/) it. This setting only has an effect if heart beat timer is active currently. The value defaults to `1800` seconds (30 minutes). Cannot be lower than 5.
