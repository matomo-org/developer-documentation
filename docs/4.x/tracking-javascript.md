...
* `setPagePerformanceTiming(networkTimeInMs, serverTimeInMs, transferTimeInMs, domProcessingTimeInMs, domCompletionTimeInMs, onloadTimeInMs)` - Manually set performance metrics in ms (thousandth of a second like `setRequestQueueInternal`) in a SPA or when Matomo cannot detect some metrics. You can set parameters to `undefined` if you do not want to track this metric. Note that `undefined` is a valid value and will not throw an error. This is by design, because users may not want to set all parameters. At least one parameter needs to be set. It will only be used on the next page view and only tracked once.
